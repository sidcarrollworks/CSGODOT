...
var _index := {}  # A mesh -> its place in _meshes.
...
## How a culled mesh casts its shadow while it is drawn, or -1 for one this
## does not cull.
func drawn_cast_shadow(mesh: MeshInstance3D) -> int:
	return _casts[_index[mesh]] if _index.has(mesh) else -1


## Changes how a culled mesh casts its shadow while it is drawn, for what
## changes the map's shadows once the culling has them (RenderVariants): at
## once if it is drawn, else when it next is. Neither to nor from OFF, since
## a mesh that casts none is hidden another way. Whether it changed.
func set_drawn_cast_shadow(mesh: MeshInstance3D, setting: GeometryInstance3D.ShadowCastingSetting) -> bool:
	var i: int = _index.get(mesh, -1)
	if i < 0 or setting == GeometryInstance3D.SHADOW_CASTING_SETTING_OFF \
			or _casts[i] == GeometryInstance3D.SHADOW_CASTING_SETTING_OFF:
		return false
	_casts[i] = setting
	if _drawn[i] != 0:
