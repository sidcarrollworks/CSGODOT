...
- Scoreboard (TAB) and buy-menu grids fit `GridContainer` or `VBox` of `HBox`. The buy menu already uses `PanelContainer` > VBox > HBox columns.
...
- `src/economy/buy_menu.gd`: a `HudElement` covering the screen, the whole menu drawn in one `_draw()` and hit-tested by hand in `_gui_input` (no `Button` nodes), over CS2's 95 % black backdrop (`.buymenu`) with each column as wide as CS2's for its kind. It sets `mouse_filter` to `STOP` only while open, and input still goes through `_input` for B, Escape and the number keys. CS2's own menu also stands your agent on the left and orders its columns in code; that pass waits for a CS2 screenshot of it.
