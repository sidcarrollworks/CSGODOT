...
- Properties: `physics_ticks_per_second` (default 60; project 64), `max_physics_steps_per_frame` (8; project 16), `physics_jitter_fix` (0.5), `time_scale` (1.0; does not change tick rate or audio speed), `max_fps` (0 = uncapped; the game sets it just under the screen's refresh as your view starts, `PlayerView.frame_cap`, unless Max FPS is set), `print_error_messages`, `print_to_stdout`.
