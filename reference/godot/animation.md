...
- Looks at odds, confirmed in a test, not fixed: `player_model.gd:771` (`AnimationNodeAdd2` "hold") and `:783` (`OneShot.MIX_MODE_ADD` "shoot") run with `deterministic` at its default false, which the AnimationMixer docs say normalises Add2 at 1.0 to the equivalent of a Blend2 at 0.5. In the docs audit, one bone at rest under a base clip that keys it and an added clip turning it 60 degrees came out at 30 with `deterministic` off and 60 with it on (60 either way when the base clip does not key the bone). So the held-gun idle and shot layers are at half strength wherever the locomotion keys the bones. `deterministic = true` also changes how missing tracks blend, so it needs a look in play.
