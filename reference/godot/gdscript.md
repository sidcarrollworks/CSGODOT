...
- Fixed in the docs audit: `ItemRegistry.weapon_data` returned `WeaponData.duplicate()` (shallow). Per `class_resource.rst` that shared the `scoped: WeaponData` sub-resource with the cached original and did not copy the non-exported lazy fields `_solved_kick_up` / `_solved_model_hold_time`, so every copy re-solved them on the first tick it was held: measured, 20 to 22 ms a gun, more than a tick. The fields are `@export_storage` now, solved when the registry builds each gun (`WeaponData._hold_times` keeps the costly one by its inputs, which 33 of the 34 guns share), and the copy is `duplicate(true)`.
