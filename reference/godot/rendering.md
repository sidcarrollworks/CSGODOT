...
- CPU cost: `rendering/occlusion_culling/occlusion_rays_per_thread` (512 × logical cores = buffer pixels) and `bvh_build_quality` (2). Both are read at start. At run time: `RenderingServer.viewport_set_occlusion_rays_per_thread()` and `viewport_set_occlusion_culling_build_quality()`. `jitter_projection` (true) hides false culls through small gaps. It shifts the buffer in a cycle of nine frames (frame 0 unshifted, the other eight diagonally by a third or a sixth of a buffer pixel each way; `RaycastOcclusionCull::_get_jitter`, read on master 2026-09-26), so a mesh whose box straddles an occluder's edge can be culled on some frames and drawn on others, with the camera still. Only what is drawn and opaque should occlude: along an invisible occluder's edge that shows as flicker (the project's dust2 sky brushes, playtest issue 11).
...
- **The simplifier ignores CUSTOM0 to 3** (verified in 4.7's `scene/resources/3d/importer_mesh.cpp`, `generate_lods`, and reproduced headless in `tests/run_map_tests.gd`'s `_test_lightmap_lods`). It welds vertices that share position, UV, UV2, normal (within the merge angle), tangent sign and colour, and every LOD's indices point at the first vertex of each welded group. A seam that only a CUSTOM channel marks, such as lightmap charts in a third UV set, is crossed in every lower LOD, and that triangle interpolates between two charts. The project draws such surfaces without LODs (`LightmapMaterials.drop_lods`, playtest of 2026-09-25 issue 12). An option that keeps LODs: at the prepare step, copy the coordinates into a channel the weld compares (COLOR_0 in the glTF), only as a key that keeps the seam, while the shader goes on reading CUSTOM0. It needs a reimport, and a colour the materials do not read.
- To drop one surface's LODs without touching its data, `ArrayMesh._get_surfaces()` and `_set_surfaces()` (bound, the pair the mesh serialises with) round-trip the raw buffers: erase the surface's `"lods"` entry and set the array back. Surface order, names, materials and bounds stay, so a MeshInstance3D's overrides still line up. Rebuilding with `surface_get_arrays` and `add_surface_from_arrays` would decompress and recompress, and appends surfaces at the end.
...
- The colour-correction LUT (`adjustment_color_correction`, a Texture3D, or a Texture2D read as three 1D curves), read from Godot 4.7.2's `servers/rendering/renderer_rd/shaders/effects/tonemap.glsl` (the source, not the docs): after the tone mapper the colour is multiplied by the brightness, converted to sRGB, pulled about 0.5 by the contrast, mixed with its RGB mean by the saturation (not luminance), clamped to 0 to 1, and used as the texture coordinate directly, with **no texel-centre offset**, so an N-cubed table's first and last texel centres sit at 0.5/N and 1 - 0.5/N and everything outside clamps. A contrast of (N - 1)/N moves 0 and 1 onto those centres (what `ColourGrade` does). The table holds sRGB: with `hdr_2d` on, Godot reads it through an sRGB view (`renderer_scene_render_rd.cpp`, `texture_get_rd_texture(..., !convert_to_srgb)`), so it comes out linear and is filtered in linear light; with it off, the value is used as it is. Only an RGBA8 (or RGB8) texture has that sRGB view. The adjustments run only with `adjustment_enabled`. Compatibility (`drivers/gles3/shaders/effects/post.glsl`) does not clamp before the table and filters it in sRGB; its grade came out differently in a cloud readback (2026-09-26), so it is no stand-in for judging Forward+'s colours.
...
- Glow shows when a pixel exceeds `glow_hdr_threshold` (1.0 = over the tonemapper's white) or when `glow_bloom` > 0. It blends Screen by default (`glow_blend_mode` 1, `GLOW_BLEND_MODE_SCREEN`, `class_environment.rst`; this page said Softlight until 2026-09-25). Glow levels 2 (0.8) and others are defaults, and level 1 does nothing in Compatibility.
...
- Ambient: `AMBIENT_SOURCE_BG` 0 (default), `DISABLED` 1, `COLOR` 2, `SKY` 3. `ambient_light_sky_contribution` 1.0 ignores `ambient_light_color`. Reflections: `REFLECTION_SOURCE_BG` 0, `DISABLED` 1, `SKY` 2. Only ReflectionProbe, VoxelGI, SDFGI or a shader writing `RADIANCE` give local reflections. `rendering/reflections/sky_reflections/texture_array_reflections` (true) "uses texture arrays instead of mipmaps for reflection probes and panorama backgrounds (sky). This reduces jitter noise and upscaling artifacts on reflections" (`class_projectsettings.rst`); on dust2 at 4K it cost 0.18 ms of GPU (measured, reference/rendering.md R5).
