...
- `reference/rendering.md`: what a frame costs to draw on Sid's RTX 4070 Ti, the suspects, and items R0 to R6 and L1 to L7. As of main 86e73c2 it is **not on main yet**. It is on the branches `claude/render-performance-rid6x7` and `claude/baked-sun-shadows-rid6x7`, with `scripts/profile_render.gd`, `src/map/render_variants.gd`, `src/map/map_shadows.gd`, `src/map/map_occluders.gd`, `src/map/world_visibility.gd` and `tests/run_render_checks.gd`. Read it there before starting render work.
...
- Cloud threads cannot measure the GPU. `--headless` "disables all rendering and window management functions. Most functions from RenderingServer will return dummy values" (`classes/class_renderingserver.rst`). `get_rendering_device()` returns `null` headless. Frame times, draw counts and video memory come only from Sid's machine (`scripts/profile_render.gd` on the render branch). Headless tests can check what the scene is set to (layers, masks, cast_shadow, materials), not what it costs.
- Read `msaa_3d` as an enum, not a sample count. `project.godot` has `anti_aliasing/quality/msaa_3d=2`, and `Viewport.MSAA_4X` = 2 (`MSAA_2X` = 1) (`classes/class_viewport.rst`). The project renders **4x MSAA**, not the 2x that `reference/rendering.md` and `RenderVariants` ("2x MSAA off") say. That is the same as Sid's CS2 setting, which changes that page's conclusion that "CS2 draws twice our MSAA samples".
- Read `soft_shadow_filter_quality` as an enum too. `project.godot` sets `directional_shadow/soft_shadow_filter_quality=4`, which is `SHADOW_QUALITY_SOFT_HIGH`. `SOFT_ULTRA` is 5 (`classes/class_renderingserver.rst`). The rendering page and `MapLighting`'s comment call it "Ultra, the highest". Soft High also multiplies the constant blur by 1.5x (Ultra 2x).
- To draw only moving things into a light's shadow map, use `Light3D.shadow_caster_mask` ("the light will only cast shadows using objects in the selected layers"; it does not change what shadows fall onto). Not `light_cull_mask`: "objects disabled via this cull mask will still cast shadows" (`tutorials/3d/lights_and_shadows.rst`). The baked-shadows branch does this with `MapShadows.LAYER` (`1 << 10`).
...
| `Environment.ssao_radius` | 1.0 | The project sets 24. |
| `rendering/environment/ssao/fadeout_from` / `fadeout_to` | 50 / 300 | **Not overridden.** SSAO fades from 4 ft and is gone at 25 ft. The same holds for `ssil/fadeout_*`. |
...
- SSAO acts on *ambient* light only, unless `ssao_light_affect` > 0. The project sets `ambient_light_disabled` on its map shaders and brings baked light through `light()`, so SSAO has little ambient to darken on those surfaces (inferred). SSAO, SSIL and SSR run at half resolution by default.
...
On main (86e73c2), verified by grep:
- Sun, sky and environment: `src/map/map_lighting.gd` (`build`: `DirectionalLight3D` with 4 blended splits, `directional_shadow_max_distance` 8192, pancake 4096, `light_angular_distance` from `angulardiameter`; `PanoramaSkyMaterial`/`ProceduralSkyMaterial`; ambient colour or sky; `REFLECTION_SOURCE_SKY`; ACES; depth fog; SSAO radius 24; glow; saturation). Lamps: `barn_light` builds `SpotLight3D`s with inverse-square attenuation and `shadow_bias` 1.0.
- Project settings: `project.godot` has `directional_shadow/size=8192`, `soft_shadow_filter_quality=4` (Soft High), `msaa_3d=2` (4x). On the render branch it also has `occlusion_culling/use_occlusion_culling=true` and `[shader_globals] probe_sun_visibility`.
...
On the render branches (not yet on main): `scripts/profile_render.gd` (`viewport_set_measure_render_time`, `viewport_get_render_info` per pass, `RENDER_*_MEM_USED`); `src/map/render_variants.gd` (toggles `msaa_3d`, `use_occlusion_culling`, the filter quality and `shadow_caster_mask`); `src/map/map_occluders.gd` (`ArrayOccluder3D.set_arrays`); `src/map/map_shadows.gd` (`LAYER = 1 << 10`); `src/map/map_lighting.gd:94` (`shadow_caster_mask = 0xFFFFFFFF & ~MapShadows.LAYER`, `light_angular_distance = 0.0`).

Looks at odds with the docs (not verified on a GPU):
- `project.godot` `msaa_3d=2` is 4x MSAA. `reference/rendering.md` (branch) says "MSAA 2x" throughout, and `render_variants.gd:23` labels `no_msaa` "2x MSAA off". Its measured `no_msaa` saving (1.7 ms at 4K) is for 4x.
- `project.godot` `soft_shadow_filter_quality=4` is Soft High, not "Ultra, the highest" (`reference/rendering.md` table; the `map_lighting.gd:69` comment "the soft filter at its highest").
- `rendering/environment/ssao/fadeout_from/to` are still 50/300. At inch scale, SSAO is fully faded 300 units (25 ft) from the camera, so most of dust2 gets none. The same applies to SSIL if it is ever enabled. Consider scaling both by 39.37 (a look choice for Sid).
...
- `src/map/far_materials.gd:52` (main) returns far depth 1.0 under Compatibility. `internal_rendering_architecture.rst` says all renderers use reverse Z. `renderers.rst` says Compatibility is "without reverse Z". The branch changed it to 0.0 after drawing through Compatibility. See `shaders.md`.
