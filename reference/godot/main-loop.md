...
- Set `physics/common/physics_jitter_fix` to `0` (inferred as the fix; not done yet). The docs recommend 0 "when using a custom physics interpolation solution, or within a network game"; this project is both and leaves it at the default 0.5 (see "Where the code already does this").
...
- `Engine.get_physics_interpolation_fraction()` returns how far the current frame is between the last two physics ticks (0..1). It is only meaningful in `_process`, and is what this project's own interpolation reads.
...
- `src/sim/sim_clock.gd`: integer microseconds from the tick count. `:68` `draw_usec()` reads `Engine.get_physics_interpolation_fraction()` for presentation. The doc comment at `:9` notes that nothing in the simulation calls `Time.get_ticks_usec()`.
...
- Custom interpolation from `Engine.get_physics_interpolation_fraction()`: `src/sim/sim_clock.gd:68`, `src/player/player_view.gd:241`, `:425`, `:508`, `src/bots/bot.gd:225`, `src/combat/ragdoll.gd:228`, `src/game/dropped_item_view.gd:77`, `src/grenades/grenade_view.gd:115`.
...
- `project.godot` `[physics]` (lines 188-198) sets neither `common/physics_jitter_fix` nor `common/physics_interpolation`, so the jitter fix stays at 0.5. The code does its own interpolation from `Engine.get_physics_interpolation_fraction()` (`src/sim/sim_clock.gd:68` and the views above), and the game will be networked; the Engine and ProjectSettings docs recommend 0 for both cases. Looks at odds, not verified (measure frame pacing before and after on Sid's machine).
