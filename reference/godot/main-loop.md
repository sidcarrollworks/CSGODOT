...
- Threaded loading:
  ...
  - Used by `RigModel.read_ahead(paths)`: the views ask for what a first buy or drop would read (every first-person clip of what your side can hold, a dropped gun's model) while the game plays. `RigModel.preload_scene` takes a scene up with `load_threaded_get`, and `RigModel.reading` polls the status, so a view can wait a frame rather than block. Measured on Sid's machine: 263 clip scenes in 188 ms on the workers, 34 MB held (they hold no images); a gun model is 49 MiB of textures and 3 of mesh.
  - Every first upload of a texture or buffer goes through a transfer worker, one per upload in flight up to the processor count, each keeping a staging buffer in system memory sized to its largest upload until the renderer shuts down (4.7.2-stable `rendering_device.cpp`). Reading 34 gun models at once on workers left 0.7 to 1.0 GiB of them, which `RENDER_VIDEO_MEM_USED` counts (it sums every Vulkan heap, system memory included). Read texture-heavy scenes with `load()` or one at a time (`reference/research/weapon-preload.md`).
  - (observed) Headless, gun models read on worker threads printed RID errors once from the dummy renderer; the views read ahead only with a display (`DisplayServer.get_name() != "headless"`), and headless they load as before.
