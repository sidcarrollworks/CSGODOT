...
- **The 3D skybox's depth squeeze is in the vertex stage** (R2). `far.gdshader` and the variants `FarMaterials.variant_of` builds set `POSITION = far_position(clip, CLIP_SPACE_FAR, PROJECTION_MATRIX)`, which keeps early depth rejection. It sets the clip depth to `far * w` plus a thousandth of the eye's clip depth from the far plane (`far_eye_depth`, read off `PROJECTION_MATRIX`), the depth an infinitely far plane would give, so a far mesh past `Camera3D.far` lands inside it rather than being clipped (playtest of 2026-09-25, issue 23); each far mesh also has a `custom_aabb` (`FarMaterials.CULL_BOX`), since the CPU culls against the far plane before any shader runs. Writing `POSITION` leaves `VERTEX` alone, and the fog's aerial perspective picks its mip from the view-space `vertex.z` (`fog_process` in 4.7.2's scene shaders), not from the depth, so the squeeze does not change the fog; writing `DEPTH` per fragment, as before, cost 0.8 ms at 1080p. A shader with a `vertex()` of its own still gets `DEPTH = far_depth(FRAGCOORD.z)` at the top of `fragment()`, unconditionally, so it meets the "all branches" rule.
