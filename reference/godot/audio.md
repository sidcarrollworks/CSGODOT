...
- Put the bus layout in `res://default_bus_layout.tres` (`audio/buses/default_bus_layout`), committed. There is none today, so only `Master` exists.
...
- No `default_bus_layout.tres`, no `AudioListener3D`, no areas with audio settings, no `AudioServer` calls: everything plays on `Master`.
