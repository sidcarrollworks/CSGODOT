...
- `hit_back_faces`: "If true, the query will hit back faces with concave polygon shapes with back face enabled or heightmap shapes." (inferred, not verified) The docs tie back-face hits to shapes with `backface_collision` on. Jolt may apply `hit_back_faces` to one-sided trimeshes too. Code that needs front faces only should set `hit_back_faces = false` explicitly, as `Hitscan._find_exit` does, rather than rely on the shape flag.
