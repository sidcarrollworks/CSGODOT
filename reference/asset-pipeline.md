...
| dust2 brush volumes | Yes, a glTF each | The buy zones, bomb sites, callouts and dust2's other brush entities are models of their own in the map's VPK, `maps/de_dust2/entities/unnamed_*.vmdl_c`, which the entity lump names in each entity's `model`: `func_bomb_target` A `unnamed_2_23316` and B `unnamed_2_23326`, `func_buyzone` T `unnamed_2_23465` and CT `unnamed_2_23467`, and the 43 `env_cs_place`. `scripts/extract_assets.sh volumes` exports all 52. Each is an empty glTF and a `_physics.gltf` holding the volume as one soup of triangles, in Source's axes and inches about the entity's origin, a brush's twelve triangles in a run. `BrushVolume` splits the soup into its closed solids (50 models are one brush, 2 are two: A site is an L), places them by the entity's origin, angles and scales, and tests points against them. The world export's own `world_physics.gltf` carries the same volumes merged, without saying which is which. |
| dust2 baked bomb damage | Yes, as KV3 text; partly worked out | `maps/de_dust2/baked_bomb_damage.vdata_c` (906 KB), `CS2_BOMB_DAMAGE_DATA` version 1, from the July 2026 bomb rework. `bombsites` is each site's box (Source mins and maxs) and damage power, 14 floats: A 1929 and B 3234, as their `func_bomb_target` entities say. `positions` is 85,697 points of three int16s each, on a 10-unit grid over the playable map (every coordinate ends in 5), stored in a hashed order. `damage_values` is 8 bytes a point, 4 a site, which read as neither floats, halves nor integers that fall off with distance: not worked out. The entity lump adds `bombradius` 700 (`info_map_parameters`) and a `cs2_bomb_damage` entity with 3000 for each site. `scripts/extract_assets.sh volumes` fetches it. |
