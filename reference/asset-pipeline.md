...
`radar`, `layers`, `sky`, `skybox`, `lightmaps`, `visibility`, `list-map` and `all`) take
...
| dust2 3D skybox | Yes, glTF | A separate map (`maps/prefabs/de_dust2/de_dust2_skybox.vpk`) built at a sixteenth of the scale around a `sky_camera` at (-8, -308, 202). Imported through MapImporter scaled by 16 about that point, with no collision. Its ground and far buildings are named `*_skybox`, which is why "skybox" is no longer a hint to hide a material; sky domes are found by their `sky.vfx` shader instead. Source 2 draws it in a pass of its own before the world, depth cleared in between; here it is ordinary geometry, and its terrain, at the skybox's own ground level, showed through wherever the map's floor lies below that (eye height under CT spawn's awning, floor at -70). So its materials are replaced by ones that squeeze their depth against the far plane (`src/map/far_materials.gd`, `far.gdshaderinc`, `MapImporter.behind_everything`): the map wins the depth test everywhere and the skybox still sorts among its own parts. Which end of the buffer is far depends on the renderer (Forward+ and Mobile reverse it), which the material is told. A blended material is cut at its edge there instead (`FarMaterials.BLENDED_CUT`), since the squeeze is written in the opaque pass; drawn solid, the palms' tree cards were triangles. It is lit as a map is: its VPK carries its own `irradiance` and `directional_irradiance` lightmaps, which `scripts/extract_assets.sh skybox` fetches beside its world (not its light probes, which would light nothing), and without which its two-layer walls had the sun alone and were black in shade. Its `direct_light_shadows` (512 square, one channel: R, G and B alike) comes with them and shadows its buildings from the sun (`MapLoader.make_skybox`), the only shadow they have, since the skybox casts nothing live. Its colour textures come out of the export without their alpha at present (see the list below: Source 2 Viewer 20.0 and CS2's version 72 shaders), and the extraction puts it back on the 9 its cut and blended materials use, so its palm, bush, olive and antenna cards are cut, not drawn whole. What the map draws in front of it is decided by the map's visibility (below): from T spawn, the kasbah towers at B would stand in front of its dome, and CS2 does not draw them from there. |
...
| View model | Assembled at runtime | Each first-person clip's glTF carries two rigs: the 56-bone view-model rig the arms follow and the weapon's own rig (bolt, magazine, trigger, muzzle), as siblings. Nothing in the clip places the weapon rig; in the game it hangs off the arm rig's `wpn` bone, so `ViewModel` pins it there on every skeleton update. The agent's arm meshes are skinned to bones the view-model rig lacks (the forearm twist bones carry weight; spine and legs are bound with none), which are added to the rig at setup under their nearest present ancestor. The clips never key the twist bones; CS2 poses them from the hands and upper arms by the tilt-twist constraints in the agent's `.vmdl`, and so does `TwistModifier` here (`reference/research/twist-constraints.md`). Bone names match across agent, rig and weapon, so no retargeting. The shared `_default_rifle` clips carry the M4A1-S weapon rig and are that weapon's clips; `rifle_ak` is the AK's. |
| Third-person model | Assembled at runtime | The world locomotion clips (`animation/anims/world/rifle/_default_rifle/`) carry the 74-bone world rig and no weapon rig, with locomotion in place (no root motion track). `PlayerModel` adopts the agent's `thirdperson_*` meshes onto that rig, keeps the weapon on its own skeleton and pins its root bone to the rig's `wpn`. The agent's bind pose and the world rig's rest pose agree bone for bone (checked), which is what makes the adoption exact. The clips are played by an `AnimationTree` built from CS2's own blend spaces (`reference/animgraph/locomotion.json`), sharing the `AnimationPlayer`'s library, which is left inactive. Two things about it: a fresh tree's first `advance(0)` only builds its caches, so posing it at once takes two (`PlayerModel.pose_now`); and the locomotion clips each carry one sync event over the whole clip (runs 0.733 s, walks 1 s, crouching 0.8 s), which is how CS2 keeps them in step. A gun's own world set (`rifle/rifle_ak`: draw, idle, reload, shoot, standing and crouched) carries its own rig's tracks too, which the body drops. Its idle and shoot clips are additive: bare differences, near-identity rotations and zero translations and scales, which played whole fold the body up; each such clip has a `+non_additive` copy beside it, which is how they are told apart. Godot adds a clip's difference from the bone's rest pose, so the body re-expresses them from the rest as they load (`PlayerModel.rest_relative`). |
| First-person body | Assembled at runtime | The same model on the local player, with `spine_2` folded to a thousandth (`RigModel.fold_bones`), and with it the chest, neck, head and arms: the skin gathers at that joint, inside the body. It stands 8 units behind the eyes, and looking down it tips back about its feet with its lower back bent forward (`LookDownArch`, a modifier after the clips), leaving the belt and legs in view. Godot's glTF import drops the clips' constant scale tracks, but the fold takes any that survive out of this model's own copies of the clips rather than trusting that; a `SkeletonModifier3D` was tried first and lost to the clip's track in 4.7's update order. |
...
| First-person animations | Yes, glTF each, every gun | CS2 keeps animations as files: `animation/anims/viewmodel/{rifle,pistol}/<set>/*.vnmclip_c`, over `animation/skeletons/characters/viewmodel.vnmskel_c` (69 bones), one set per gun (draw, idle, inspect, reload, fire; pistols add their empty-magazine clips) and the shared `_default_rifle` and `_default_pistol` sets, which are the M4A1-S's and the USP-S's; SMGs, shotguns, snipers and machine guns are all `rifle/` sets. Each exports as a glTF with one animation. The clips' names end in the set's weapon, which is not always the folder's (`pistol_glock18` holds `draw_glock`), so the view model takes the suffix most of a set's files share (`RigModel.common_suffix`); the M249's and G3SG1's idle is `idle1`. Since CS2 1.41.8.2 some clips have a compiled non-additive copy beside them, named for the clip (`prepare_shoot_revolver.vnmclip+non_additive`): the revolver has nine, as many as its own clips, which threw off the set's suffix until `common_suffix` stopped counting them; the tables leave them out too. World-model clips are under `animation/anims/world/`: each gun's draw, idle, reload and fire, standing and crouched, over the shared locomotion. `reference/weapons/models.md` lists every gun's sets, written by `scripts/weapon_tables.gd`. |
| The equipment | Yes, as the guns | `scripts/extract_assets.sh equipment` fetches the bomb (`weapons/models/c4/`, with the defuse multimeter), the defuse kit (`defuser/`), the six grenades (`grenade/<kind>/`, the molotov with its broken glass), the two default knives (`knife/knife_default_{ct,t}/`) and the Zeus (`taser/`), with their first- and third-person sets, skeletons and the clips' own data; the defuse is a third-person set of its own, `world/shared/defuse/`, a clip for each kind of weapon in hand. `sounds` fetches each one's sound folder. Two things differ from the guns: the default knives' export has only the body the guns carry a spare of for legacy skins (`body_legacy`, `reference/research/weapon-preload.md`), which `RigModel.is_spare_body` keeps where it is the only one; and the T knife's clips end in two words, `_default_t`, which `RigModel.common_suffix` takes off whole. `reference/weapons/equipment.md` lists it all, with the game's numbers and the clips' timings. |
...
| Animation graphs | Yes, as text (`-b DATA`) | CS2's AnimGraph 2, `animation/graphs/*.vnmgraph_c`: 232 compiled graphs, the logic that plays the clips (the third-person player's, its locomotion, each weapon's first- and third-person, the inspects, the chicken, the menus). `scripts/extract_assets.sh animgraphs` prints them all in seconds; `NmGraph` reads them and `scripts/animgraph_tables.gd` writes `reference/animgraph/`. A graph compiled with other clips is a variation, `<graph>.vnmgraph+<variation>`. It is Esoterica's animation graph under Valve's names, so Esoterica's source says what each node does; `reference/animgraph2.md` has the rest. |
...
| dust2 lighting | Sun, sky, fog, exposure | The glTF's one light gives the sun's direction; `light_environment`, `env_sky`, `env_cubemap_fog` and the `post_processing_volume` in the entity lump give its colour and brightness, the sky material (exported as an equirect HDR `.exr`), the haze and the exposure window. Shadows: the world's surfaces are one-sided and a wall often has no face on its far side (a room's wall is the face that looks into the room), so with Godot's default of casting front faces only, the sun came through every wall that faced away from it (B site's back corner, CT's awning, the tunnels). The map casts `SHADOW_CASTING_SETTING_DOUBLE_SIDED` (`MapImporter.cast_shadows`), since R4 into the lamps' shadows only wherever its shadow from the sun is baked; the 3D skybox casts nothing, because cast double-sided its hills' back faces shadowed half the map. The shadow range is 8192 units (the map is ~5,000 across), the map 8192 square, the soft filter at its highest, or the sun's quarter-degree penumbra comes out as dither. The lamps: of dust2's 31 lights besides the sun (25 `light_rect`, 4 `light_barn`, 2 `light_omni2`), all but two are baked whole into the lightmap (`directlight 1` with no baked shadow index: Source 2 Viewer's `SceneLight.GetCost` calls them static), so their pools of light, CT spawn's among them, are already there. The two lamps down lower tunnels are stationary (`directlight 3`, baked shadow indices 1 and 2): the lightmap holds only their bounce light and a shadow mask, and CS2 lights them as it draws, which left the tunnel dim here. `MapLighting.add_lamps` builds each as Source 2 Viewer shades a barn light (`lighting.barn.slang`): a frustum whose eye is `1 / size_params.z` (8.33 units) behind the lamp and `size_params` (16) wide either side there, 62.5 degrees off its axis; its `brightness_lumens` over the frustum's solid angle (3.62 sr), 40 pi lumens a steradian at one unit, falling off as the inverse square from the eye; faded over its outer `soft_x`. That is the unit of the sun's brightness, so it takes the sun's 0.7: 72,874 at one unit for the 3000-lumen lamp, 1.6 on the floor 212 units below against the sun's 1.75. A Godot spot light from the eye does it, reaching 2.5 times CS2's range because Godot fades a light over all of its range and CS2 does not, with Godot's shadow in place of the baked mask. Three things it took: Godot's cone fade (`spot_angle_attenuation`) is its own curve, not a rim exponent, and measured on a plane its default of 1 keeps the full light to about three quarters of the angle, near CS2's soft third, where 9 left a third of the light 30 degrees in and the tunnel dim; the lamp's glowing fixture (`F_SELF_ILLUM`) encloses its bulb and would shadow its light entirely, so it casts no shadow (`MapImporter`), as CS2's baked shadow leaves it out; and a depth bias of one unit (`MapLighting.LAMP_SHADOW_BIAS`), where Godot's 0.03, meant for metres, striped the walls and blacked out the floor with the lamp's own shadow (normal bias alone, at 4, did not clear it). Measured against Sid's CS2 screenshot of lower tunnels from its mouth, its wall 1.7 times darker here and its floor 0.8 times; the mouth's own arch piers and the tunnel's ceiling, which only baked light reaches, are 2.5 to 4.5 times darker, where CT spawn's baked lamps and roof come within 1.35 to 1.7 times of his: a question for the baked light there, not the lamps. Other live kinds (rect, omni) are counted and not built; dust2 has none. |
| dust2 lightmaps | Yes, `.exr` + `.png` | `maps/de_dust2/lightmaps/irradiance.vtex_c` (8192 square, BC6H, 78 MB; the sky and bounce light reaching each surface, HDR) and `directional_irradiance.vtex_c` (4096 square: which way that light mostly comes from, in tangent space, and how directional it is). The sun is not in them: CS2 computes it live, with `direct_light_shadows` as the static map's shadow from it, fetched since R4 (`reference/rendering.md`): one channel per stationary light, 1 where it is blocked, the light's `bakedshadowindex` naming its channel (the sun's is 0 on dust2, the lamps down lower tunnels 1 and 2). Every lightmapped material takes the sun times one minus the sun's channel, as Source 2 Viewer's `lighting.slang` does (`MapShadows`, `lightmap.gdshaderinc`), and the map is then left out of the sun's live shadow map. It is imported at BC7 without the alpha border fix (`write_import_settings.gd`), since each channel is a different light's shadow. Extracted on Sid's machine (2026-09-25) it is an 8192-square RGBA PNG, alpha 254 or 255, the sun's channel 255 over 73.4% of it and 0 over 18.0%; Godot's first import of it finished and then crashed on its way out, so `import_assets` (`scripts/common.sh`) runs an import that fails once more. Every world surface carries lightmap coordinates in the glTF's second UV set, already scaled by Source 2 Viewer to the texture (its `GetScaledLightmapUvAccessor`); a debug render of the game's own chart-colour map (`debug_chart_color.vtex_c`) over the geometry lands one colour on each face with the seams at the edges, which is the alignment check. The shading is Source 2 Viewer's `ComputeLightmapShading` less its curvature term. The lightmap's result is handed to Godot as the surface's ambient light (`IRRADIANCE`, `baked_light.gdshaderinc`), which Godot multiplies by the occlusion and by the albedo as the decals left it: added as emission, as it first was, it was fixed before the decals and a bullet hole could not darken it. Until `reference/rendering.md` R5 it was added in the light pass with Godot's ambient turned off (`ambient_light_disabled`), which took every reflection with it. The `light()` there writes out Godot's own Burley and GGX terms, since defining it replaces them, for the sun's baked shadow. `lightmap_energy` 0.4 scales the lightmap's result into Godot's units alongside the sun (measured against a CS2 screenshot of B site: shaded ground 0.27 in both, shaded wall 0.24 against 0.20). |
| dust2 lightmapped props | Yes, in the same maps | Props are `csgo_vertexlitgeneric` (and foliage), merged by model into aggregate nodes, and each placed prop is baked one of three ways, which its second UV set tells: as lightmap charts, laid out at the world's texel density (0.75 a unit on dust2; 1,238 of the 2,010 merged primitives, measured over a sample of triangles, within an eighth to four times the world's median); collapsed onto a single texel of its own in a dense block at one corner of the atlas, which holds that instance's light (the map's light-probe result, baked); or absent. A few models carry their own second UV set instead, for a decal or a tint mask, and their materials say so with `F_FORCE_UV2` in the vmat's `IntParams`; that set is never read as a lightmap's, whatever its density. Most are ten to forty times denser than a lightmap (`dust_arch_01_pattern`, `dust_wall_braces`), but the `dust_shipping_crate_01` set sits at one to two texels a unit, inside the lightmap window, and read as lightmaps it wore blotches of someone else's light where its stickers go: the flag, not the density, is the rule. Where the map lightmapped such a prop, the export puts its lightmap coordinates in a third set (`TEXCOORD_2`, in the range of the others' lightmap charts), which Godot imports as `CUSTOM0`, two floats a vertex; `LightmapMaterials` lights it from there (`lightmap_uv_in_custom0`). On dust2 that is 187 surfaces: the kasbah towers, the arches, the crates and the wall braces. Lit instead by one probe sample a piece, as they were, the towers and the CT arch wore blocks of different orange where the pieces met, and a piece whose probe sat inside a wall was black. Godot's import-time LODs weld vertices without comparing CUSTOM0, so the lower LODs of these surfaces crossed their lightmap seams and wore zigzag stripes at range; `LightmapMaterials.drop_lods` draws them at full detail at every distance, the rest of their mesh keeping its LODs (playtest of 2026-09-25, issue 12). The lit props are 1,496 surfaces on dust2 (the probes light 943 more), the awnings' planks and posts among them. |
| dust2 light probes | Yes, 720 `.exr` slices | 43 `env_combined_light_probe_volume` entities, each a box (`box_mins`/`box_maxs` about `origin`) of cells `voxel_size` (24) across, all baked into one 3D atlas, `lightmaps/env_light_probe_volume_atlas.vtex_c`, 192 × 176 × 720: six bands of 120 slices, one per face of an ambient cube in Source's axes (+X +Y +Z -X -Y -Z), each volume's block placed by `light_probe_atlas_x/y/z` and sized `light_probe_size_x/y/z` in texels (the entity keys Source 2 Viewer's `WorldLoader` reads; the maths is its `lighting.lpv.slang`). The `_dlshd` atlas beside it holds the same lights' shadows at each cell, in the same channels as `direct_light_shadows` and placed by the same keys, one band deep (Source 2 Viewer's `SceneLightProbe` reads it that way; the extraction's count line says whether dust2's is): fetched since R4, its slices go under `lightmaps/probes/` with the atlas's, and `LightProbes` packs the sun's channel into `lightprobe_sun.bin`, a byte a cell of how much sun gets through, drawn as one 3D texture that every probe-lit shader reads at each pixel; the octree `.dat`s are the game's own lookup and are not needed. Source 2 Viewer decompiles the atlas to one HDR image per slice; they go under `lightmaps/probes/` with a `.gdignore`, and `LightProbes` packs them into `lightprobes.bin` (RGB half floats, 146 MB) on first load. The rows are as stored: checked against the lightmap at 379 world triangles (a surface's probe shade against its lightmap irradiance correlates 0.82 that way, 0.41 flipped within the block, 0.11 flipped over the image; the means agree, 1.48 against 1.45, which also fixes the energy at the lightmaps'). A point takes the smallest volume around it, read trilinearly. `ProbeMaterials` puts the props the lightmaps did not cover (1,100 surfaces) on `probe_lit.gdshader` with the cube as instance uniforms, sampled once at each mesh's centre, or, for a mesh over 256 units across (a prop the export merged from copies all over the map), averaged over the cubes 4 units out from a spread of its own vertices (`ProbeMaterials.light_points`, `cube_for`): the middle of dust2's 760-unit window meshes is inside a building, where the probes are black, and every window was. Such a mesh reads the sun's baked shadow in the volume holding the most of those points (`shadow_for`), not the one round its middle; `RigModel.light_from` does the same for the players' models every frame. The lightmap's average (`lightmaps/average.json`) stays as the ambient for the far skybox and anything else. |
| dust2 visibility | Yes, compiled and as KV3 text | `maps/de_dust2/world_visibility.vvis_c` (3.9 MB): which parts of the map can be seen from which, what CS2 culls the world by. `scripts/extract_assets.sh visibility` keeps it compiled and decompiles it beside it: the text is the data block, with the counts and offsets (4096 clusters, 512 bytes a row, an octree over [-8768, -7136, -7768] to [7616, 9248, 8616] in Source's axes); the rest is the `VXVS` block, read straight out of the file. `WorldVisibility` follows Source 2 Viewer's reading (`VoxelVisibility.cs`): an octree whose 72,472 leaves (32 to 4096 units) are diced into 4 × 4 × 4 cells, each cluster in a leaf covering a 64-bit mask of them; a cluster's row is a bit for every cluster it sees; a mesh belongs to every cluster its box touches and is drawn while any is in the camera's cluster's row (`Scene.IsNodeInPvs`). 17,270 octree nodes list the clusters inside them; each list is what walking its leaves finds (all of dust2's checked), so a box that covers a node takes it, and a box over 1024 units on two axes takes the list of any node it reaches, as Source 2 Viewer does. From Sid's T spawn spot, 333 clusters are seen and 2,702 of the 3,589 world meshes are not drawn, the kasbah towers at B among them, which drawn stood in front of the skybox's dome (CS2 shows the dome there). A mesh not drawn keeps its shadow (`SHADOWS_ONLY`, one-sided where the map's own are two-sided; rendered from four of Sid's spots, only what the culling took away changed); one that casts none, a lamp's fixture, is hidden. |
...
| dust2 brush volumes | Yes, a glTF each | The buy zones, bomb sites, callouts and dust2's other brush entities are models of their own in the map's VPK, `maps/de_dust2/entities/unnamed_*.vmdl_c`, which the entity lump names in each entity's `model`: `func_bomb_target` A `unnamed_2_23316` and B `unnamed_2_23326`, `func_buyzone` T `unnamed_2_23465` and CT `unnamed_2_23467`, and the 43 `env_cs_place`. `scripts/extract_assets.sh volumes` exports all 52. Each is a glTF, empty but for the two `func_brush` (which are seen), and a `_physics.gltf` holding the volume as one soup of triangles, in Source's axes and inches about the entity's origin, a brush's twelve triangles in a run. `BrushVolume` splits the soup into its closed solids (50 models are one brush, 2 are two: A site is an L), places them by the entity's origin, angles and scales, and tests points against them. The world export's own `world_physics.gltf` carries the same volumes too, one node per entity in the lump's order, but named only by class (`func_buyzone`, `env_cs_place`), without the keys that say which side's zone or which site. |
| dust2 baked bomb damage | Yes, as KV3 text; partly worked out | `maps/de_dust2/baked_bomb_damage.vdata_c` (906 KB), `CS2_BOMB_DAMAGE_DATA` version 1, from the July 2026 bomb rework. `bombsites` is each site's box (Source mins and maxs) and damage power, 14 floats: A 1929 and B 3234, as their `func_bomb_target` entities say. A's box is the bounds of its L, notch included, so where a bomb may be planted is the volume, not this box. `positions` is 85,697 points of three int16s each, on a 10-unit grid over the playable map (every coordinate ends in 5), stored in a hashed order. `damage_values` is 8 bytes a point, 4 a site, which read as neither floats, halves nor integers that fall off with distance: not worked out. The entity lump adds `bombradius` 700 (`info_map_parameters`; despite the name the old bomb's damage, which reached 3.5 times as far) and a `cs2_bomb_damage` entity with 3000 for each site. `scripts/extract_assets.sh volumes` fetches it. |
| CS2 surfaces | Yes, as KV3 text | In the main archive: `surfaceproperties/surfaceproperties.vsurf_c` (164 surfaces, each with its physics, friction among it, the hash of its name, `m_nameHash`, and its parent by that hash, `m_baseNameHash`, and by name too for most) and `scripts/surfaceproperties_game.txt` (78 of them listed, 77 with values, `chain` with none: the material letter, jump and speed factors, climbable, `allowsmokethrough`, and the two penetration modifiers, 84 of those; each gives only what differs from its parent). `scripts/extract_assets.sh surfaces` fetches both and writes `reference/surfaces/surfaces.csv` (committed, as the game gives them) and `surfaces.md` (resolved); `SurfaceProperties` reads the csv. The collision hull's physics names the surfaces it uses by the same hashes (`m_surfacePropertyHashes`: 31 slots, 30 surfaces, Wood twice), and Source 2 Viewer writes a part whose surface it has no name for as `vrf_unknown_key_` and that hash: dust2's railings are `physics_group_vrf_unknown_key_2838185980`, metalrailing. The physics also gives a surface to each triangle of a mesh (`m_Materials`), which the export drops: dust2's ground mesh comes out all concrete, though about 23,000 of its 175,380 triangles are other surfaces, sand, default, gravel, dirt and tile the most (roadmap 7c). |
| dust2 radar | Yes, png + text | In the main archive, not the map's: `panorama/images/overheadmaps/de_dust2_radar_psd.vtex_c` (1024 square) and `resource/overviews/de_dust2.txt`, which puts the image's top-left corner at Source (-2476, 3239), 4.4 units a pixel, Source +X across and -Y down, and marks where the loading screen shows the spawns and sites. `scripts/extract_assets.sh radar` fetches both; `MapOverview` goes from game space to the image and back. |
| Sounds | Yes, one file each | Each sound is a `.vsnd_c` that decompiles to the audio it holds (wav, a few mp3), a few numbered variants to a set: `sounds/weapons/ak47/ak47_01..04` are the AK's shots, `sounds/player/footsteps/sand_01..12` the sand steps, `land_sand_*` the landing. `scripts/extract_assets.sh sounds` fetches every gun's sounds and the equipment's (each one's folder: the bomb's beeps, plant, defuse and blast, the grenades' pins, throws, bounces and detonations, the fire's loop, the knives' swings and hits, the Zeus; 33 MB), footsteps and landings for the surfaces the hull names, and the hit feedback CS2's attacker feedback events name (`kevlar_01-08`, `headshot_noarmor_01-05`, the helmet's `headshot_armor_e1` and `headshot_armor_flesh`, and the body's `physics/surfaces/mud_impact_bullet1-4`, with what the one hit and those near hear, `player_damagebody_04-08`, and the death groan, `death1-6`; until 2026-09-24 it fetched the older `kevlar1-5`, `headshot_armor_01` and `bodyshot_kill_01`, which no CS2 event plays) (at first the two rifles' alone: 284 files, 19 MB). `SoundBank` finds a set by the stem its variants share and plays it through one `AudioStreamRandomizer`, so shots overlap. The sound event definitions (`soundevents/game_sounds_weapons.vsndevts_c`) that carry the game's volumes and distances are not extracted from the game: GameTracking-CS2 publishes them as text, and `scripts/sound_events.sh` turns them into `reference/sounds/sound_events.json` (since 2026-09-26; the older sounds' levels are still by ear). Footsteps map the hull part's material name to a set (`Footsteps.SURFACE_SETS`); Godot's 3D audio is in metres, so the players' `unit_size` is 10 m in inches. Every gun's whole folder is fetched since 2026-09-22 (the M4A4 and M4A1-S share `m4a1/`, the MP5-SD's is `mp5/`, the USP-S's `usp/`), with the weapon sounds they share (empty clicks, zoom, the fire-mode switch); `reference/weapons/sounds.md` sorts them by role. With 743 files the path list overran Windows' 32,767-character command line, so the extraction passes it in batches (`s2v_batched`). Bullet impacts are `sounds/physics/{concrete,surfaces,metal,wood}/*_impact_bullet*` by surface, mapped from the hull part's name like the footsteps; bright ticks with most of their energy above 4 kHz, played at -20 dB and fading over 80 m, since at the shot's own level they read as the gun's sound, a rifle that rattles like gravel. |
| Tracers and muzzle flashes | Their textures, png; the effects' numbers by hand from their `.vpcf` | CS2's particle systems do not run in Godot. `scripts/extract_assets.sh effects` fetches the eleven textures the tracers and flashes draw with into `assets/effects/`; the sheets (flames, steam, smoke) come out a trimmed png a frame, and `scripts/effect_textures.gd` puts every frame back where the texture's `-b DATA` block says it was, with the frames' rectangles in `<name>.sheet.json`. The effects themselves (`particles/weapons/cs_weapon_fx/`, `particles/unified_weapon_fx/`) decompile to KV3 text with `-d`; their numbers are in `Tracers` and `FlashTable` (`reference/weapons/effects.md`). |
...
- **Props carry a decal and a glow the glTF has no slot for.** CS2's prop
  shaders lay a decal texture over the colour (`F_DECAL_TEXTURE`,
  `g_tDecal`: the dirt down the kasbah towers, the crates' stencils) and add
  self-illumination (`F_SELF_ILLUM`, `g_tSelfIllumMask`: the hanging lamps in
  lower tunnels and elsewhere). `scripts/extract_assets.sh layers` fetches
  both textures with the second layers, and `prop_features.gdshaderinc`
  follows Source 2 Viewer's `ApplyDecalTexture` and
  `GetStandardSelfIllumination`: the decal mixed in by its alpha
  (`F_DECAL_BLEND_MODE` 0) or multiplied in (1), through the second UV set
  where the material says so; the glow 2 to the `g_flSelfIllumBrightness`,
...
- **A Source 2 Viewer older than CS2's shaders exports the wrong channels.**
  ...
  which Source 2 Viewer 20.0 cannot read ("Only VCS file versions 59 through
  71 are supported", once per material in the export's output). It then
  guesses, and writes every colour texture without its alpha: an alpha-cut
  or translucent material draws whole. dust2's own export predates the
  update and is right (its 64 such materials all have alpha); the skybox,
  exported since, lost it on all 11. Support is in Source 2 Viewer's master
  since 23 September 2026, not yet in a release. Until then the extraction
  puts it back: after the `layers` and `skybox` steps, `scripts/export_alpha.gd`
  lists the colour images of cut and blended materials that have no alpha,
  with the texture each came from (the material's `g_tColor`), and those are
  decompiled again on their own, which keeps the alpha, over the export's
  copies (9 on the skybox; the two other cut materials share their images).
