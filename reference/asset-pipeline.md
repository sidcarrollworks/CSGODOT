...
| CS2 surfaces | Yes, as KV3 text | In the main archive: `surfaceproperties/surfaceproperties.vsurf_c` (164 surfaces, each with its physics, friction among it, and its parent by the hash of its name, `m_baseNameHash` against `m_nameHash`, and by name too for most) and `scripts/surfaceproperties_game.txt` (the game's values for 78 of them: the material letter, jump and speed factors, climbable, `allowsmokethrough`, and the two penetration modifiers; each gives only what differs from its parent). `scripts/extract_assets.sh surfaces` fetches both and writes `reference/surfaces/surfaces.csv` (committed, as the game gives them) and `surfaces.md` (resolved); `SurfaceProperties` reads the csv. The collision hull's physics names the 31 surfaces it uses by the same hashes (`m_surfacePropertyHashes`), which is how the one Source 2 Viewer cannot name on dust2, `vrf_unknown_key`, is known to be `metalrailing`. |
