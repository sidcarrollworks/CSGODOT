...
func _play_on(map: MapContents, team_size: int, practice: bool = false, warmup_seconds: float = 0.0, side: String = "T") -> Array:
...
func _test_the_bomb_each_round(side: String) -> void:
	var played := _play_on(_small_map(), 2, false, 0.0, side)
	var world: GameWorld = played[1]
	var mode: Competitive = played[2]
