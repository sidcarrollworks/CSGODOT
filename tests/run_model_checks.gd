...
func _start_combat() -> void:
	...
	# The player wears the capsules the bot does, on a body nobody sees: the
	# view draws its own.
	var hidden := _victim.model != null
	if hidden:
		for mesh in _victim.model.find_children("*", "MeshInstance3D", true, false):
			hidden = hidden and not (mesh as MeshInstance3D).visible
	...
	_check(
		...
		"the player wears CS2's nineteen capsules too (%d), on a body that is not drawn, the head %.0f up (%s)"
